...
public class Echiquier implements BoardGames {
	...
	private boolean isMoveLegal(int xInit, int yInit, int xFinal, int yFinal) {
		...
		if (!(xInit == xFinal && yInit == yFinal)) {
			...
			// s'il existe une pièce intermédiaire sur la trajectoire --> false
			// (sauf cavalier)
			...
			if (coordPieceInter != null) {
				...
				// pièce intermédiaire sur la trajectoire qui empêche
				// déplacement
				else {
					isPieceOnPath = true;
				}
			...
			// si position finale ne correspond pas à algo de déplacement piece
			// --> false
			boolean isMoveJeuOk = jeuCourant.isMoveOk(xInit, yInit, xFinal,
					yFinal, this.isCatchOk, this.isCastlingPossible);

			this.isMoveOk = this.isPieceToMoveOk && !isPieceOnPath
					&& isMoveJeuOk;
		}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see java.lang.Object#toString() invoque la méthode toString des 2 jeux
	 */
	...
	public List<PieceIHM> getPiecesIHM() {
		List<PieceIHM> piecesIHM = jeuCourant.getPiecesIHM();
		for (PieceIHM piece : jeuOppose.getPiecesIHM()) {
			piecesIHM.add(piece);
		}
		return piecesIHM;
	}
